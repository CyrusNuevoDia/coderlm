...
class RLM(Module):
    ...
    def _make_llm_tools(self, max_workers: int = 8) -> dict[str, Callable]:
        ...
        def llm_query_batched(prompts: list[str]) -> list[str]:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_idx = {
                    executor.submit(_query_lm, p): i for i, p in enumerate(prompts)
                }
    ...
    def _prepare_execution_tools(self) -> dict[str, Callable]:
        ...
        execution_tools.update(
            {name: tool.func for name, tool in self._user_tools.items()}
        )
        return execution_tools

    def _inject_execution_context(
        self, interpreter: CodeInterpreter, execution_tools: dict[str, Callable]
    ) -> None:
    ...
    def _process_execution_result(
        ...
        if isinstance(result, FinalOutput):
            parsed_outputs, error = self._process_final_output(
