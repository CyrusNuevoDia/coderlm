...
```bash
./src/coding-agent-rlm claude "**/*.ts" --prompt "Summarize" --max-depth 1
```
...
**Problem**: When running `coding-agent-rlm` with Gemini from inside another agent session (e.g., Claude Code), the shared `nia` MCP server can't handle concurrent connections. Gemini hangs during startup trying to connect.
